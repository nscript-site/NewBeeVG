...
public class NBGrid : NBPanel
{
    public static void SetColumn(NBVisual element, int value)
    {
        if(element.ParamsInGrid == null)
            element.ParamsInGrid = new NBParamsInGrid();
        element.ParamsInGrid.Column = value;
    }

    public static int GetColumn(NBVisual element)
    {
        return element.ParamsInGrid?.Column ?? 0;
    }

    public static void SetRow(NBVisual element, int value)
    {
        if (element.ParamsInGrid == null)
            element.ParamsInGrid = new NBParamsInGrid();
        element.ParamsInGrid.Row = value;
    }

    public static int GetRow(NBVisual element)
    {
        return element.ParamsInGrid?.Row ?? 0;
    }

    public static void SetColumnSpan(NBVisual element, int value)
    {
        if (element.ParamsInGrid == null)
            element.ParamsInGrid = new NBParamsInGrid();
        element.ParamsInGrid.ColumnSpan = value;
    }

    public static int GetColumnSpan(NBVisual element)
    {
        return element.ParamsInGrid?.ColumnSpan ?? 1;
    }

    public static void SetRowSpan(NBVisual element, int value)
    {
        if (element.ParamsInGrid == null)
            element.ParamsInGrid = new NBParamsInGrid();
        element.ParamsInGrid.RowSpan = value;
    }

    public static int GetRowSpan(NBVisual element)
    {
        return element.ParamsInGrid?.RowSpan ?? 1;
    }
    ...
    public NBColumnDefinitions ColumnDefinitions
    {
        get
        {
            if (_extData == null) { _extData = new ExtendedData(); }
            if (_extData.ColumnDefinitions == null) { _extData.ColumnDefinitions = new NBColumnDefinitions() { Parent = this }; }
    ...
    /// </summary>
    public NBRowDefinitions RowDefinitions
    {
        get
        {
            if (_extData == null) { _extData = new ExtendedData(); }
            if (_extData.RowDefinitions == null) { _extData.RowDefinitions = new NBRowDefinitions() { Parent = this }; }
    ...
    protected override void ArrangeChilds(Point origin, Size arrangeSize)
    {
        try
        {
            ...
            if (_extData is null)
            {
                ...
                {
                    var child = children[i];
                    child.TryArrange(new Rect(origin, arrangeSize));
                }
            }
            else
            {
                ...
                for (int currentCell = 0; currentCell < PrivateCells.Length; ++currentCell)
                {
                    ...
                    cell.TryArrange(origin, cellRect);
                }
    ...
    /// </summary>
    private IReadOnlyList<NBDefinitionBase> DefinitionsU
    {
    ...
    /// </summary>
    private IReadOnlyList<NBDefinitionBase> DefinitionsV
    {
    ...
    /// </summary>
    private NBDefinitionBase?[] TempDefinitions
    {
        get
        {
                ...
                || extData.TempDefinitions.Length < requiredLength)
            {
                ...
                {
                    extData.TempDefinitions = new NBDefinitionBase[requiredLength];
                    Thread.SetData(s_tempDefinitionsDataSlot, new WeakReference(extData.TempDefinitions));
                }
                else
                {
                    extData.TempDefinitions = (NBDefinitionBase[]?)tempDefinitionsWeakRef.Target;
                    if (extData.TempDefinitions == null
                        || extData.TempDefinitions.Length < requiredLength)
                    {
                        extData.TempDefinitions = new NBDefinitionBase[requiredLength];
    ...
    /// </summary>
    private static double StarWeight(NBDefinitionBase def, double scale)
    {
        if (scale < 0.0)
        {
    ...
    {
        if (ColumnDefinitionsDirty && _extData != null)
        {
            var extData = _extData;
            ...
            if (extData.ColumnDefinitions == null)
            {
                ...
                {
                    extData.DefinitionsU = new NBDefinitionBase[] { new NBColumnDefinition() { Parent = this } };
                }
            }
            else
            {
    ...
    {
        if (RowDefinitionsDirty && _extData != null)
        {
            var extData = _extData;
            ...
            if (extData.RowDefinitions == null)
            {
                ...
                {
                    extData.DefinitionsV = new NBDefinitionBase[] { new NBRowDefinition() { Parent = this } };
                }
            }
            else
            {
    ...
    private void ValidateDefinitionsLayout(
        IReadOnlyList<NBDefinitionBase> definitions,
    ...
    internal double GetFinalColumnDefinitionWidth(int columnIndex)
    {
        ...
        {
            IReadOnlyList<NBDefinitionBase> definitions = DefinitionsU;
        ...
        double desiredSize)
    {
        ...
        {
            NBDefinitionBase?[] tempDefinitions = TempDefinitions; //  temp array used to remember definitions for sorting
            int end = start + count;
        ...
        double availableSize)
    {
        ...
        NBDefinitionBase?[] tempDefinitions = TempDefinitions;
        ...
        {
            NBDefinitionBase def = definitions[i];
    ...
    internal double GetFinalRowDefinitionHeight(int rowIndex)
    {
        ...
        {
            IReadOnlyList<NBDefinitionBase> definitions = DefinitionsV;
    ...
    {
        internal NBColumnDefinitions? ColumnDefinitions;  //  collection of column definitions (logical tree support)
        internal NBRowDefinitions? RowDefinitions;        //  collection of row definitions (logical tree support)
        internal IReadOnlyList<NBDefinitionBase>? DefinitionsU;    //  collection of column definitions used during calc
        internal IReadOnlyList<NBDefinitionBase>? DefinitionsV;    //  collection of row definitions used during calc
        ...
        internal NBDefinitionBase?[]? TempDefinitions;              //  temporary array used during layout for various purposes
    ...
    private class SpanPreferredDistributionOrderComparer : IComparer
    {
        ...
        {
            NBDefinitionBase? definitionX = x as NBDefinitionBase;
            NBDefinitionBase? definitionY = y as NBDefinitionBase;
    ...
    {
        private readonly IReadOnlyList<NBDefinitionBase> definitions;

        internal MinRatioIndexComparer(IReadOnlyList<NBDefinitionBase> definitions)
        {
        ...
        public int Compare(object? x, object? y)
        {
            ...
            NBDefinitionBase? definitionX = null;
            NBDefinitionBase? definitionY = null;
    ...
    private class MaxRatioIndexComparer : IComparer
    {
        ...
        internal MaxRatioIndexComparer(IReadOnlyList<NBDefinitionBase> definitions)
        {
            this.definitions = definitions ?? throw new ArgumentNullException(nameof(definitions));
        }
    ...
    private class StarWeightIndexComparer : IComparer
    {
        ...
        internal StarWeightIndexComparer(IReadOnlyList<NBDefinitionBase> definitions)
        {
            this.definitions = definitions ?? throw new ArgumentNullException(nameof(definitions));
        }
    ...
    /// </summary>
    internal class GridLinesRenderer : NBVisual
    {
        /// <summary>

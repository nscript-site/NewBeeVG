...
namespace NewBeeVG.Core.Controls.Layout;

/// <summary>
/// Provides helper methods needed for layout.
/// </summary>
public static class NBLayoutHelper
{
    /// <summary>
    ...
    public static Size ApplyLayoutConstraints(NBLayoutable control, Size constraints)
    ...
    private static Size ArrangeChildInternal(NBLayoutable? child, Size availableSize, Thickness padding)
    {
    ...
    /// </summary>
    public static void InvalidateSelfAndChildrenMeasure(NBLayoutable control)
    {
        void InnerInvalidateMeasure(NBVisual target)
        {
            if (target is NBLayoutable targetLayoutable)
            {
            ...
            {
                NBVisual child = visualChildren[i];

                InnerInvalidateMeasure(child);
            }
        }

        if (control is NBVisual v)
